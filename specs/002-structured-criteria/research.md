...
**補足**: 終了コード 127（コマンド未検出）は仕様上 not met として扱うが、コマンド名の typo による意図しない「not met」の繰り返しを早期発見するため、warning ログを出力する。
...
- `DEFAULT_COMMAND_TIMEOUT_SEC = 30.0`（コマンド実行のデフォルトタイムアウト）
