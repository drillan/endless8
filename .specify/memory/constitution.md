...
Added sections:
  ...
  - Governance (Amendment Procedure, Versioning Policy, Compliance Review, Application Principles)
Removed sections: N/A (新規作成)
Templates requiring updates:
  - .specify/templates/plan-template.md: ✅ 整合性確認済み（Constitution Checkセクション対応）
  - .specify/templates/spec-template.md: ✅ 整合性確認済み
  - .specify/templates/tasks-template.md: ✅ 整合性確認済み（TDDワークフロー対応）
Follow-up TODOs: None
-->

# endless8 Constitution
...
3. テストファイル名は対象機能を明確に反映させなければならない（MUST）
   - 例: `test_engine.py` ← `engine.py`
   - 例: `test_reception_agent.py` ← `reception_agent.py`
...
### Article 3: Agent Architecture Compliance（エージェントアーキテクチャ準拠）

すべてのエージェント実装は、設計原則に準拠しなければならない（MUST）。

**4エージェント構成の原則**:
1. **受付エージェント**: 完了条件の妥当性チェック、曖昧な場合は質問生成
2. **実行エージェント**: タスク実行に専念、毎回フレッシュな状態で開始
3. **サマリエージェント**: 実行結果を圧縮して履歴に保存
4. **判定エージェント**: 完了条件の評価

**エージェント設計要件**:
- エージェントは単一責務に従わなければならない（MUST）
- エージェント間の通信は明確なインターフェースを通じて行わなければならない（MUST）
- 各エージェントは独立してテスト可能でなければならない（MUST）
- pydantic-ai のモデル定義を使用しなければならない（MUST）

**コンテキスト管理要件**:
- 実行エージェントは毎イテレーションでコンテキストをリフレッシュしなければならない（MUST）
- 履歴はサマリ化して効率的に参照しなければならない（MUST）
- コンテキスト枯渇を防ぐ設計としなければならない（MUST）

**理由**: エージェント構成の一貫性と長時間実行での安定性を保証
...
**フレームワーク信頼の原則**:
- pydantic-ai の機能を直接使用しなければならない（MUST）
- 不必要なラッパーを作成してはならない（MUST NOT）
- claudecode-model アダプタを標準的に活用しなければならない（MUST）
...
```python
# 悪い例（禁止）
max_iterations = 10  # ハードコードされた値
if not history:
    history = []  # 暗黙的フォールバック

# 良い例（推奨）
MAX_ITERATIONS = int(os.environ["E8_MAX_ITERATIONS"])  # 環境変数から取得
if not history:
    raise ValueError("History is required for execution")  # 明示的エラー処理
```
...
async def run_task(input: TaskInput) -> ExecutionResult:
    """タスクを実行し、結果を返す"""
    ...

# 悪い例（禁止）
def run_task(input):  # 型注釈なし
    return {"success": True}  # 型なし辞書を返す
```
...
- 例: `001-engine-core`, `002-agent-reception`, `003-history-management`
...
- エージェントアーキテクチャ原則に準拠しているか（Article 3）
